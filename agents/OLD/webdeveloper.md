...
<!--
  Historical note (no action required from you): in session 2026-04-25-2,
  vibe-5 contained an injected director-bridge `<script>` block that
  silently broke save-edits in the parent app for hours. Root cause and
  fix were on the iframe-loading side of the system, not in this agent.
  Kept here so future maintainers know the incident exists.
-->
