...
export function splitOnQuestionForms(input: string): FormSegment[] {
  ...
  while (cursor < input.length) {
    ...
    if (form) {
      ...
      // Parse failure — most common cause is a FALSE-POSITIVE open tag,
      // e.g. an inline backticked mention of `<question-form>` in prose
      // where the user is discussing the tag itself. The prior behavior
      // consumed the whole span up to the next `</question-form>` close,
      // which would swallow a REAL block that came LATER in the message
      // and prevent it from ever being parsed (Ralph 2026-05-06 — CD's
      // message had both an inline mention and a real block; only the
      // mention rendered as code, the real block never reached the form
      // renderer because the parser's failure path consumed it).
      //
      // Recovery: keep the failed open tag as text (preserves prose
      // verbatim, including the inline backticked mention) and advance
      // the cursor only past the tag itself. The regex scan on the next
      // iteration picks up at openEnd, finds the real block, and parses
      // it cleanly.
      out.push({ kind: 'text', text: input.slice(openStart, openEnd) });
      cursor = openEnd;
    }
  }
  // Adjacent-text consolidation. The failed-parse recovery (above) can
  // emit two adjacent text segments (the failed open tag + the trailing
  // text after the next iteration finds no more opens). Merge them so
  // downstream consumers see one continuous prose chunk — this also
  // preserves the legacy invariant that a single malformed block round-
  // trips as one text segment.
  return mergeAdjacentText(out);
}

function mergeAdjacentText(segments: FormSegment[]): FormSegment[] {
  const merged: FormSegment[] = [];
 ...
 * picks visible.
 *
 * Lives here (rather than in the React component) so it's testable without
 * pulling in JSX / DOM.
 */
