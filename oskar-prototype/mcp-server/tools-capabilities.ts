...
export const CAPABILITY_TOOL_DEFINITIONS = [
  // ── Tier S: generate_image ────────────────────────────────────────────
  {
    name: 'generate_image',
    description:
      'Fire Nano Banana directly without waiting for the user to click Generate. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        refs: {
          ...
          items: { type: 'string' },
          description:
            'Optional reference image filenames in the session folder. The server ' +
        ...
        slot: {
          type: 'string',
          description:
            'Optional vibe slot the image targets (e.g. "hero", "menu-bg"). Becomes ' +
  ...
  // ── Tier S: snackbar (fire-and-forget) ────────────────────────────────
  {
    name: 'snackbar',
    description:
      'Speak unprompted to the user via a snackbar. Fire-and-forget: no return ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        severity: {
          ...
          description:
            'Default `info`. `info`=blue, `success`=green, `progress`=cyan, ' +
        ...
        sticky: {
          type: 'boolean',
          description:
            'Override the per-severity default. true → stay until dismissed; ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        options: {
          ...
          minItems: 2,
          description:
            'Discrete option strings. The user picks one. Returned verbatim.',
        },
      },
  ...
  // ── Tier A: session_meta ─────────────────────────────────────────────
  {
    name: 'session_meta',
    description:
      'Snapshot of the entire session in one call. Returns vibesBuilt, vibesPending, ' +
  ...
  // ── Tier A: list_assets (state index — no thumbnails) ────────────────
  {
    name: 'list_assets',
    description:
      'State index of every image in the session. Returns metadata only — NO ' +
  ...
  // ── Tier A: find_assets ──────────────────────────────────────────────
  {
    name: 'find_assets',
    description:
      'Keyword search over filenames + Nano CD analysis text. Returns ranked ' +
  ...
  // ── Tier A: lint_brand_compliance ────────────────────────────────────
  {
    ...
    description:
      'Lint a vibe HTML file for the v1 rule set: (1) every <img> must have BOTH ' +
  ...
  // ── Tier A: apply_patch ──────────────────────────────────────────────
  {
    name: 'apply_patch',
    description:
      'Surgically edit a built vibe HTML in place — no full rebuild. The edit ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        edit: {
          type: 'object',
          description:
            'Discriminated by `kind`. Each kind has its own required params; ' +
  ...
  // ── Tier B: image_ops ────────────────────────────────────────────────
  {
    name: 'image_ops',
    description:
      'Sharp pipeline. Operations: crop ({x,y,w,h} or {aspect}), slice ({rows,cols}), ' +
  ...
  // ── Tier B: vibe_diff ────────────────────────────────────────────────
  {
    name: 'vibe_diff',
    description:
      'Compare a vibe HTML to its `last-build` snapshot. Returns unified diff + ' +
...
export async function callCapabilityTool(
  ...
  switch (name) {
    ...
    case 'generate_image': {
      ...
      if (!r.ok) return { text: `generate_image failed: ${r.error}`, isError: true }
      if (r.body?.status === 'running' && r.body.jobId) {
        ...
        return {
          text:
            `generate_image enqueued: jobId=${r.body.jobId}${dedupNote}. ` +
    ...
    case 'screenshot': {
      ...
      if (!r.ok) return { text: `screenshot failed: ${r.error}`, isError: true }
      ...
      if (r.body?.savedPath) return { text: `Screenshot saved: ${r.body.savedPath}`, isError: false }
    ...
    case 'snackbar': {
      ...
      const sev =
        sevIn === 'success' ? 'success' :
        sevIn === 'progress' ? 'progress' :
        sevIn === 'warning' || sevIn === 'warn' ? 'warning' :
        sevIn === 'error' ? 'error' :
        'info'
      const sticky = typeof args.sticky === 'boolean' ? args.sticky : undefined
      const r = await postJson<{ ok: boolean }>('/api/mcp/snackbar', {
        sessionId,
        text,
        severity: sev,
        sticky,
      })
      if (!r.ok) return { text: `snackbar failed: ${r.error}`, isError: true }
    ...
    case 'ask_user': {
      ...
      if (options.length < 2) return { text: 'Error: need at least 2 options', isError: true }
      const r = await postJson<{ choice?: string; error?: string }>(
        '/api/mcp/ask-user',
    ...
    case 'session_meta': {
      const r = await postJson<{
        ...
        brokenRefs: string[]
        currentPhase: string
      }>('/api/mcp/session-meta', { sessionId })
      if (!r.ok) return { text: `session_meta failed: ${r.error}`, isError: true }
    ...
    case 'list_assets': {
      ...
      if (!r.ok) return { text: `list_assets failed: ${r.error}`, isError: true }
    ...
    case 'find_assets': {
      ...
      const r = await postJson<{ hits: unknown[] }>('/api/mcp/find-assets', {
        sessionId,
        query,
        limit: args.limit,
      })
      if (!r.ok) return { text: `find_assets failed: ${r.error}`, isError: true }
      return { text: JSON.stringify(r.body?.hits || []), isError: false }
    }

    case 'lint_brand_compliance': {
      const file = String(args.file || '').trim()
      if (!file) return { text: 'Error: file is required', isError: true }
      const r = await postJson<{ violations: unknown[] }>('/api/mcp/lint-brand', {
        sessionId,
        file,
      })
      if (!r.ok) return { text: `lint_brand_compliance failed: ${r.error}`, isError: true }
    ...
    case 'apply_patch': {
      ...
      const r = await postJson<{ ok: boolean; affected?: number; diff?: string; error?: string }>(
        '/api/mcp/apply-patch',
        { sessionId, target, edit },
      )
      if (!r.ok) return { text: `apply_patch failed: ${r.error}`, isError: true }
      if (!r.body?.ok) {
    ...
    case 'image_ops': {
      ...
      const r = await postJson<{
        ok: boolean
        outputs?: { filename: string; dimensions: string; sizeKB: number }[]
        error?: string
      }>('/api/mcp/image-ops', { sessionId, filename, operation, params: args.params })
      if (!r.ok) return { text: `image_ops failed: ${r.error}`, isError: true }
      if (!r.body?.ok) return { text: `image_ops error: ${r.body?.error}`, isError: true }
    ...
    case 'vibe_diff': {
      ...
      const r = await postJson<{
        diff: string
        summary: { linesAdded: number; linesRemoved: number; sectionsChanged: number }
        note?: string
        error?: string
      }>('/api/mcp/vibe-diff', { sessionId, target, since })
      if (!r.ok) return { text: `vibe_diff failed: ${r.error}`, isError: true }
      if (r.body?.error) return { text: `vibe_diff error: ${r.body.error}`, isError: true }
    ...
    default:
      return { text: `Unknown capability tool: ${name as string}`, isError: true }
  }
}
