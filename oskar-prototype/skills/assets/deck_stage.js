 ...
 * - Speaker-notes postMessage (so an outer shell can render them)
 ...
 *
 * Speaker notes: add the following inside <head>
...
(function() {
  ...
  class DeckStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // Render the Shadow DOM first (independent of child nodes — not affected by parser timing).
      this._render();

      // Defensive: if the <script> sits inside <head> (rather than after </deck-stage>),
      // the parser may not have finished the child <section>s yet, so querySelectorAll
      // would return empty. Defer to the next event loop tick so all children have
      // finished parsing.
      const init = () => {
        this._collectSlides();
      ...
      if (this.ownerDocument.readyState === 'loading') {
        // Document is still parsing; wait for DOMContentLoaded to grab every section in one shot
        this.ownerDocument.addEventListener('DOMContentLoaded', init, { once: true });
      } else {
        // Document has finished parsing (script at end of <body> or deferred) — next frame is enough
