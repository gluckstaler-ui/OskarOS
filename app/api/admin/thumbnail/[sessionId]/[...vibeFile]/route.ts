...
interface RouteParams {
  params: Promise<{
    sessionId: string
    /** Catch-all segment — array of path components (e.g. ["VIBE-1", "foo.html"]). */
    vibeFile: string[]
  }>
}
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Validate inputs
    if (!sessionId || !vibeFile || vibeFile.length === 0) {
    ...
    // Join catch-all segments back into a path (preserves subdirs like VIBE-1/foo.html)
    const vibeFilePath = vibeFile.join('/')

    // Ensure path ends with .html (allow .png too for URL cleanliness)
    let htmlFile = vibeFilePath
    if (vibeFilePath.endsWith('.png')) {
      htmlFile = vibeFilePath.replace(/\.png$/, '.html')
    } else if (!vibeFilePath.endsWith('.html')) {
      htmlFile = `${vibeFilePath}.html`
    }
