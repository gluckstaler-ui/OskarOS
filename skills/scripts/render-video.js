...
(async () => {
  ...
  if (hasReady) {
    // Second line of defense: proactively reset animation time to zero — covers
    // the case where the HTML doesn't strictly follow the starter-tick template
    // (e.g. lastTick uses performance.now() so font-load time leaks into the
    // first-frame dt). See references/animation-pitfalls.md §12 for details.
