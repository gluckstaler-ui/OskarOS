...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    let systemPrompt = buildCDPrompt(sourceImages || [], sessionId || 'default-session', isResume || false)
...
Don't be polite. Be brilliant. If something is stunning, say "holy shit." If something is mediocre, say so. I need your real creative instincts, not a sanitized assessment.`
    ...
    if (sessionId && (totalInputTokens > 0 || totalOutputTokens > 0)) {
      try {
        ...
        await appendUsage(sessionId, 'CD', { inputTokens: totalInputTokens, outputTokens: totalOutputTokens, cost }, 'Chat API call')
        console.log(`[${requestId}] Usage tracked: ${totalInputTokens}in/${totalOutputTokens}out = $${cost.toFixed(4)}`)
      } catch (usageErr) {
