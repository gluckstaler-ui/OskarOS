...
export async function parseVibesFromFiles(sessionPath: string): Promise<ParsedVibe[]> {
  ...
  try {
    ...
    if (vibeFiles.length === 0) {
      // Fallback: parse from monolithic CREATIVE-BRIEF.md
      console.log(`[parseVibesFromFiles] No VIBE-*.md files found in ${sessionPath}, falling back to CREATIVE-BRIEF.md`)
