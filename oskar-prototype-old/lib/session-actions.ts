...
import { analyze } from './nano-banana'
...
export async function uploadImageAction(
  ...
  try {
    ...
    let analysis = null
    try {
      analysis = await analyze(base64Data)
    } catch (analysisError) {
...
export async function getImageManifestsAction(
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      const vibeId = `vibe-${vibe.toLowerCase().replace(/\s+/g, '-')}`
      ...
      if (generatedVersions.length > 0) {
        ...
        const constructedFilename = `${vibe.toLowerCase().replace(/\s+/g, '-')}-${purpose}-v1.jpg`
