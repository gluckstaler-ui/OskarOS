...
export const CONSULAR_TOOL_DEFINITIONS = [
  {
    name: 'crm_query',
    description:
      'Run ONE SQL statement against the CRM database — your single tool, full ' +
    ...
    inputSchema: {
      ...
      properties: {
        sql: {
          type: 'string',
          description:
            'A single SQL statement. SELECT with explicit columns for reads; ' +
...
export type ConsularToolName = (typeof CONSULAR_TOOL_DEFINITIONS)[number]['name']

interface SqlResult {
  ok: boolean
  kind: 'read' | 'write' | 'rejected'
...
export async function callConsularTool(
  name: ConsularToolName,
  args: Record<string, unknown>,
  _ctx: ToolCallContext,
): Promise<{ text: string; isError: boolean }> {
  if (name !== 'crm_query') {
    return { text: `Unknown consular tool: ${name as string}`, isError: true }
  }
  ...
  const r = await postJson<SqlResult>('/api/admin/crm/consular/sql', { sql })
  ...
  const body = r.body
  if (!body) return { text: 'crm_query: empty response from route', isError: true }
