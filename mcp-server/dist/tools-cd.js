...
export const CD_TOOL_DEFINITIONS = [
    ...
    {
        name: 'build_vibe',
        description: 'Build N vibes from their `vibe-N-{slug}.md` specs. Array-based: pass ' +
        ...
        inputSchema: {
            ...
            properties: {
                slugs: {
                    type: 'array',
                    items: { type: 'string' },
                    description: 'Non-empty list of vibe slugs to build. Each entry is either ' +
    ...
    {
        name: 'job_status',
        description: 'Poll the status of a long-running job (build_vibe / build_wireframes / ' +
    ...
    },
    {
        ...
        description: 'Phase 1 → Phase 2 gate. Surface the brand on one screen: ' +
    ...
    // optional-empty states. Two layouts: webpage-vertical and keynote-multi-row.
    {
        ...
        description: 'Present a vibe\'s complete image plan as a card with slot states. ' +
    ...
    // user toggles between them via dropdown and picks one (or creates new).
    {
        ...
        description: 'Present N candidate design systems for the user to pick one. ' +
    ...
    // top-3 narrow, etc.). Same yellow chassis as Design Directions.
    {
        ...
        description: 'Surface the descent-selection card for vibe picking. CD specifies the ' +
    ...
    switch (name) {
        ...
        case 'build_vibe': {
            ...
            if (Array.isArray(args.slugs)) {
                ...
                slugs = [args.slug.trim()];
            }
            if (slugs.length === 0) {
                return {
                    text: 'Error: `slugs` (non-empty array of strings) is required. ' +
                        'Example: build_vibe({slugs: ["vibe-3"]}) or build_vibe({slugs: ["vibe-1","vibe-2","vibe-3","vibe-4"]}).',
                    isError: true,
                };
            }
            const r = await postJson('/api/mcp/build-vibe', { sessionId, slugs });
            if (!r.ok)
            ...
            if (Array.isArray(r.body?.jobs) && r.body.jobs.length > 0) {
                ...
                const wording = r.body.jobs.length === 1
        ...
        case 'tc_discovery': {
            ...
            const r = await postJson('/api/mcp/ask-discovery-questions', { sessionId, questions, context, title, progress, preamble });
            if (!r.ok)
                return { text: `tc_discovery failed: ${r.error}`, isError: true };
            if (r.body?.error)
                return { text: `tc_discovery error: ${r.body.error}`, isError: true };
            return {
        ...
        case 'tc_understanding': {
            ...
            const r = await postJson('/api/mcp/confirm-understanding', {
                sessionId,
                summary,
                readyToGenerate,
            ...
            if (!r.ok)
                return { text: `tc_understanding failed: ${r.error}`, isError: true };
            if (r.body?.error)
                return { text: `tc_understanding error: ${r.body.error}`, isError: true };
            return {
        ...
        case 'tc_image_strategy': {
            ...
            const r = await postJson('/api/mcp/present-image-strategy', { sessionId, slug, vibeSlug, vibeName, layout, phaseLabel, slots, preamble: args.preamble });
            if (!r.ok)
                return { text: `tc_image_strategy failed: ${r.error}`, isError: true };
            if (r.body?.error)
                return { text: `tc_image_strategy error: ${r.body.error}`, isError: true };
            return {
                text: `Image Strategy card surfaced for ${vibeName} (${layout}, ${r.body?.slotCount ?? slots.length} slots). ` +
        ...
        case 'tc_design_system': {
            ...
            const r = await postJson('/api/mcp/present-design-system', { sessionId, slug: dsSlug, vibes: dsVibes, prompt: args.prompt });
            if (!r.ok)
                return { text: `tc_design_system failed: ${r.error}`, isError: true };
            if (r.body?.error)
                return { text: `tc_design_system error: ${r.body.error}`, isError: true };
            return {
                text: `Design System card surfaced (${r.body?.vibeCount ?? dsVibes.length} vibes). ` +
        ...
        case 'tc_descent_selection': {
            ...
            const r = await postJson('/api/mcp/present-descent-selection', {
                sessionId,
                slug: dscSlug,
                cap: dscCap,
            ...
            if (!r.ok)
                return { text: `tc_descent_selection failed: ${r.error}`, isError: true };
            if (r.body?.error)
                return { text: `tc_descent_selection error: ${r.body.error}`, isError: true };
            return {
                text: `Descent selection card surfaced (${r.body?.vibeCount ?? dscVibes.length} candidates, cap=${dscCap}). ` +
