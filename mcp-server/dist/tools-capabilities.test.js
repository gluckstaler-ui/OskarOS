...
const callCapabilityTool = (name, args) => rawCallCapabilityTool(name, args, TEST_CTX);
const { postJson } = await import('./api-client.js');
const mockPost = postJson;
...
describe('snackbar', () => {
    ...
    it('preserves all 5 canonical severities', async () => {
        mockPost.mockResolvedValue({ ok: true, status: 200, body: { ok: true } });
        for (const sev of ['info', 'success', 'progress', 'warning', 'error']) {
            mockPost.mockClear();
...
describe('callCapabilityTool — unknown tool', () => {
    it('returns isError for unknown name', async () => {
        const r = await callCapabilityTool('definitely_not_a_capability', {});
        expect(r.isError).toBe(true);
        expect(r.text).toContain('Unknown');
    });
});
//# sourceMappingURL=tools-capabilities.test.js.map