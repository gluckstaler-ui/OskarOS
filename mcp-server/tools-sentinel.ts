...
export const SENTINEL_TOOL_DEFINITIONS = [
  {
    ...
    description:
      'Call AFTER writing the narrative critique. Submits the structured ' +
    ...
    inputSchema: {
      ...
      properties: {
        target: {
          type: 'string',
          description:
            'What you critiqued — e.g. "vibe-3.html", "CREATIVE-BRIEF.md", ' +
            '"vibe-3 + IMAGES.md".',
        },
        scores: {
          type: 'array',
          items: {
            ...
            properties: {
              dimension: {
                type: 'string',
                description:
                  'The huashu dimension scored: "philosophy_alignment", ' +
        ...
        summary: {
          type: 'string',
          description:
            'One-paragraph summary of the verdict. The narrative text in ' +
        ...
        recommendations: {
          ...
          items: { type: 'string' },
          description:
            'Specific, actionable recommendations. Each one a single ' +
...
export type SentinelToolName = (typeof SENTINEL_TOOL_DEFINITIONS)[number]['name']

export async function callSentinelTool(
  ...
  switch (name) {
    ...
    default:
      return { text: `Unknown Sentinel tool: ${name as string}`, isError: true }
  }
}
