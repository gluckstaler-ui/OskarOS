...
- **Chat surface doctrine and renderer must ship together — or the doctrine ships first.** When the chat-surface UI gains new channel renderers (TodoWrite panel, ToolCard inline cards, `<question-form>` interactive forms — open-design port Phase 2, Commits A + C), the agent prompts that EMIT into those channels MUST be updated BEFORE the renderer ships. Reverse order = UI looks broken because half its features have nothing to render (CD still emits numbered-bold questions and prose-narrates every tool call → no question-form to parse, no tool-call card to inline). Same shape as the build_vibe wrapper bug: two state machines (agent doctrine + UI parser) silently drift unless one side commits first. The doctrine side is cheaper to commit (zero LOC, prompt edit) and forward-compatible (raw JSON / raw XML in chat is ugly but harmless until the renderer lands). Logged 2026-05-02 ahead of Phase 2. Trigger: any time WebDev or Jedi Code lifts new chat-surface components from open-design or huashu, the agent doctrine for the matching emission pattern goes into `creative-director-agent.md` / `webdev-agent.md` / `sentinel-ti.md` / `jedi-code` rules in the SAME PR or earlier. Don't ship parallel.

- **If a tool that should render as a card produces no card or wrong card, log it.** Once the 13-tool ToolCard custom-card surface ships (CD: 8, WebDev: 1, Sentinel: 1, cross-agent: 3), every MCP call from any agent that doesn't land in its declared card surface is doctrine drift. Don't paper over with "the generic fallback card is fine" — file a notify_agent to Jedi Code and an INSTITUTIONAL-MEMORY entry. The cards ARE the visual feedback loop; trusting a generic fallback for a known tool is the same anti-pattern as trusting a JSON-stringified tool_result instead of a typed render.

- **CD's MCP tool surface lives in TWO independent allowlists. Both must be updated, or new tools silently disappear.** (1) `lib/mcp-config.ts` `CD_ALLOWED_TOOLS` — passed to the `claude` subprocess as `--allowed-tools` at spawn time, restricts what CD's *process* may call. (2) `mcp-server/tools.ts` `CD_ALLOWED` — used by `listToolsForRole('cd')`, restricts what the MCP server *advertises* in `tools/list` responses to a CD-role connection. Server-side advertisement is the gate that determines whether a tool reaches CD's deferred-tool list at all. If a name is in (1) but not (2), CD's spawn will allow the call but the server never tells her the tool exists. The reverse leaves the tool visible in tools/list but rejected at spawn time. Symptom: "I added the tool to the allowlist but CD still can't see it." Logged 2026-05-06 after `ask_discovery_questions` / `confirm_understanding` / `propose_image_prompt` / `todo_write` / `report_build_*` / `submit_critique` were all in the spawn list but missing from the server filter. Same shape as the build_vibe wrapper drift, the agent-relay drift, the one-to-many addressing drift — protocol on one end, contract on the other, no shared schema enforcing parity. **Fix in this iteration**: explicit comment in BOTH files saying "edit the other one too." **Right fix**: factor both into a single source — e.g. each tool definition in `mcp-server/tools-{cd,webdev,sentinel}.ts` carries its own `roles: ['cd']` field, and the spawn-side allowlist + the server-side filter both derive from that single field. Eliminates the drift class entirely.
...
- **`tc_design_directions` is TRACK-AGNOSTIC. Never add a `track` field — anywhere.** Design Directions close Discovery BEFORE the project's build axis (webpage / keynote / brand-cards) has been committed. The track lives on `tc_understanding` (the Phase 1 → Phase 2 gate), not on the directions card. Re-introducing a `track` parameter into the directions schema couples a not-yet-decided downstream concern into an upstream gate, and 400s the validator when a careless caller omits it. This rule has been broken **twice**: once silently read from the old route file into doctrine, then a second time when refactoring the schema reference table in `agents/creative-director-agent.md`. Same shape as the route-wrapper drift: doctrine and validator co-evolve unless something enforces the match — here, nothing did. Logged 2026-05-14. **If you find yourself adding `track:` to anything under `present_design_directions` / `tc_design_directions` / the `design_directions` event payload / `DesignDirectionsCard` props / the schema-reference table in CD's doctrine — STOP and re-read this rule before committing.**

- **`tc_design_directions` is cap-4, not cap-2. Card UI, route validator, doctrine, and CD preamble copy must all agree.** Locked 2026-05-18 in `agents/creative-director-agent.md`. Code-side reconciliation 2026-05-20: `components/chat/DesignDirectionsCard.tsx` was cap-2 client-side (lines 9, 106, 136, 150, 169), `app/api/mcp/present-design-directions/route.ts` doc said "up to 2". CD fired the card with a "MAX 4" preamble, user could only pick 2, then had to type the missing 2 picks into chat — the exact UX failure the card exists to prevent. Same drift class as the `track` field (entry above) and the `build_vibe` cap (entry below): doctrine and component co-evolving without a shared schema. **If you find yourself touching the cap number on this card — `next.size < N`, `PICK ≤N`, `Selected: X / N`, "Pick up to ___" prompt copy, or the doctrine entry in CD-agent.md — change ALL of them in the same commit. Doctrine says 4. Code must follow.** Discovered + fixed 2026-05-20.

- **Before firing any `tc_*` card, `grep cap components/chat/<CardName>.tsx`.** The card components hardcode caps client-side. The CD doctrine table is the source-of-truth, but it CAN drift from the implementation (see entries above). A 4-second grep before firing prevents the entire class of "preamble said N, UI enforced M" failures. CD-MEMORY.md gets the same rule for the CD-only practice log.

- **Two build-tool surfaces only: `build_wireframes` (Phase 2) and `build_vibe` (Phase 4 + Phase 5). Don't reintroduce `build_all_vibes` or `build_final` — they collapse into array-based `build_vibe`.** As of 2026-05-18, `build_vibe` takes `slugs: string[]`. Single rebuild = `build_vibe(["vibe-3"])`. Batch = `build_vibe(["vibe-1", "vibe-2", "vibe-3", "vibe-4"])`. Phase-5 final ship = `build_vibe([selectedSlug])` — strictness is derived by the orchestrator from session state (selection lock + approved image canon), NOT by a separate tool. `build_wireframes` keeps its own surface because its lifecycle (loose strictness, WebDev self-critique, no Sentinel) is different. Same drift class as `track`-on-`tc_design_directions`: doctrine in CD-agent.md / WebDev-agent.md / workflow.md / slide-decks.md, validator schemas in `mcp-server/tools-{cd,orchestrator}.ts`, allowlists in `mcp-server/tools.ts` + `lib/mcp-config.ts`, escrow JobKind in `lib/build-escrow.ts` — six surfaces, no shared schema, drift on every refactor. **If you find yourself reaching for `build_all_vibes` or `build_final` — STOP. The contract is `build_vibe(slugs[])` for both shapes. The orchestrator decides Phase-4-vs-Phase-5 strictness; CD just passes the right slug set.** Logged 2026-05-18.

---

## Failure Log

### 2026-05-20 — Doctrine drift: `tc_design_directions` cap-4 in doctrine, cap-2 in code

**Session:** 2026-05-20-2 (Oppliger). Ralph caught it after CD fired the card with a "MAX 4" preamble and the UI only allowed 2 picks.
**Symptom:** CD fired `tc_design_directions` with 6 strategic-bet tiles and a preamble instructing Robert to "Pick MAX 4." The rendered `DesignDirectionsCard` enforced `cap-2` client-side. Robert picked 2, then had to type the missing 2 picks into chat ("die weiteren zwei sind: Konventionell, Kachelofen"). The card exists specifically to prevent the prose-fallback UX failure; here it caused one.
**Turns burned:** 1 silent + 1 user-correction + 1 push-back ("you used the wrong design-directions toolcard"). Three rounds before fix landed.
**Root cause:** Same shape as the `track` drift entry below. Doctrine locked cap=4 on 2026-05-18 in `agents/creative-director-agent.md` (overview + canonical-payload table). The cap-2 enforcement in `components/chat/DesignDirectionsCard.tsx` (lines 9, 106, 136, 150, 169) and the doc comment in `app/api/mcp/present-design-directions/route.ts` line 6 were never updated to match. CD trusted the doctrine without grep-verifying the implementation before firing.
**Fix (2026-05-20):**
- `components/chat/DesignDirectionsCard.tsx` — updated doc header (lines 4–9), changed `next.size < 2` → `next.size < 4` (line 106), pill text `PICK ≤2` → `PICK ≤4` (line 136), fallback prompt "Pick up to two" → "Pick up to four" (line 150), counter `Selected: {selected.size} / 2` → `/ 4` (line 169).
- `app/api/mcp/present-design-directions/route.ts` — doc comment updated to "up to 4 (cap locked 2026-05-18, code-side fix 2026-05-20)" with anchor to this entry.
- `docs/INSTITUTIONAL-MEMORY.md` — Don't-Do List entry added (cap-N reconciliation rule + grep-before-fire rule).
- Doctrine in `agents/creative-director-agent.md` is the source of truth; no change needed there.

**Lesson — promoted to the Don't-Do List.** Two rules: (1) the cap-N reconciliation rule (doctrine + card + route + preamble must agree, change in one commit), (2) the grep-before-fire rule (4-second verification step before any `tc_*` call). Same meta-shape as the `track` drift and the `build_vibe`/`build_all_vibes`/`build_final` drift: N surfaces describing one contract, no shared schema enforcing parity. Until a typed source-of-truth lands, the Don't-Do rules + the doctrine comments in each touched file are the next line of defense.

**Tags:** `#doctrine-drift` `#tc_design_directions` `#cap-mismatch` `#two-state-machines` `#cd-agent` `#mcp` `#preamble-ux`

---

### 2026-05-14 — Doctrine drift: `track` grafted onto `tc_design_directions` (2nd time)

**Session:** 2026-05-14, in-chat. Ralph caught it; the violation had ALREADY been corrected once in a prior session and re-introduced.
**Symptom:** Ralph re-read `agents/creative-director-agent.md` and found the canonical-payload table claiming `tc_design_directions` requires a `track` field with enum `'webpage' | 'keynote' | 'brand-cards'`. The validator in `app/api/mcp/present-design-directions/route.ts` 400'd any caller that omitted `track`. The runtime `DesignDirectionsCard` rendered `Closes Discovery · Track: {track}` in the head-bar. The schema, the route, the preview-card validator, the page.tsx event handler, the React component, the `DesignDirectionsCardPayload` type — all six locations had been re-coupled to a `track` field that Design Directions is **track-agnostic** about by deliberate design.
**Turns burned:** 0 in *this* session (Ralph spotted it on read-through). But this is the **second** time the same bug shipped. The first occurrence was silent: an earlier Jedi Claude read the (incorrect) `track` field out of the route file and propagated it into the doctrine table. Ralph corrected that. Then a *subsequent* Jedi Claude — refactoring the same table for another change — re-read the route file, saw `track` still there in the validator, and faithfully re-introduced it into the doctrine table as "required." Same shape, second iteration, no checkpoint catching the drift.
**Root cause:** **doctrine and validator co-evolving without a shared schema, plus an upstream/downstream conflation in the workflow itself.** Two failures stacked:

1. **Workflow-level conflation.** Design Directions close Discovery BEFORE the build axis has been committed. The track (`webpage` / `keynote` / `brand-cards`) is decided downstream of Directions, on the `tc_understanding` gate that pre-fills from Discovery answers AND lets the user flip the radio before clicking Build. Coupling `track` into the upstream gate inverts the dependency: now CD must KNOW the track to even fire the directions card, when in fact picking directions is *part of* discovering what the track should be. The `track` field on `present_design_directions` was design debt left over from an earlier sketch where directions and track committed together.

2. **No source-of-truth enforcement.** The (mis)schema lived in `mcp-server/tools-orchestrator.ts` (tool definition), `app/api/mcp/present-design-directions/route.ts` (route validator), `app/api/mcp/preview-card/route.ts` (preview validator), `lib/types.ts` (`DesignDirectionsCardPayload`), `components/chat/DesignDirectionsCard.tsx` (React props + render), `app/page.tsx` (event-handler card-message assembly), AND `agents/creative-director-agent.md` (CD's reference table). Seven locations, hand-synced. When the first Jedi Claude pulled the field out of doctrine, four of the seven still carried it. The next refactor re-propagated.

**Fix (2026-05-14):**
- `mcp-server/tools-orchestrator.ts` — removed `track` property from `tc_design_directions` schema; removed from `required`; removed track variable + validation + payload in the dispatcher; removed track from the response text; **added an in-file doctrine comment** in the dispatcher pointing at this entry.
- `app/api/mcp/present-design-directions/route.ts` — removed `track` from body type, validation, and `publish()` payload; **added a doctrine comment** in the file header.
- `app/api/mcp/preview-card/route.ts` — removed the `'design_directions'` track validator; **added an inline doctrine comment**.
- `lib/types.ts` — removed `track` from `DesignDirectionsCardPayload`; **added a doctrine comment** on the type.
- `components/chat/DesignDirectionsCard.tsx` — removed the `track` prop from the interface, the destructure, and the head-bar render (`Closes Discovery · Track: {track}` → `Closes Discovery`); **added a doctrine comment** on the props interface.
- `app/page.tsx` — removed the track-bucket assignment in the `design_directions` case of the SSE handler; **added a doctrine comment**.
- `agents/creative-director-agent.md` — rewrote the schema reference table row for `tc_design_directions` to declare the correct payload AND lead with the bold rule "TRACK-AGNOSTIC — never add a `track` field."

**Lesson — promoted to the Don't-Do List.** Same meta-shape as the build_vibe wrapper drift and the CD allowlist drift: when N locations describe the same contract and nothing enforces parity, every refactor risks resurrecting the deleted field somewhere. **The fix in this commit is tactical** — six files edited, plus an INSTITUTIONAL-MEMORY anchor. The **structural fix** is the same as the others: derive route validators, MCP schemas, React types, and doctrine tables from a single typed source so the drift is impossible. Until that lands, the Don't-Do entry is the next line of defense: any future agent editing the design-directions schema reads the bold "TRACK-AGNOSTIC" rule at every touch point in the codebase before saving.

**Why this is logged as a 3+ turn bug despite zero in-session turns:** the bug shipped TWICE silently. First introduction + correction + second silent re-introduction + Ralph's catch on doctrine re-read = four agent-rounds, three of them invisible until Ralph noticed. Total burned: Ralph's bandwidth twice, plus the implicit cost of the validator-rejected `tc_design_directions` calls CD made before figuring out it needed `track`. Logging.

**Tags:** `#doctrine-drift` `#tc_design_directions` `#track-agnostic` `#two-state-machines` `#workflow-conflation` `#cd-agent` `#mcp` `#repeat-offender`
