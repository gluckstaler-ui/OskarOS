...
❌ PARTIAL   Sage 240/40 cut — DETERMINISTIC boundary selection SHIPPED 2026-04-25
             (JS does anchor + 200-line window + snap-to-turn-seam; agent only
             writes Block-narrative prose). Time-based protection heuristic
             REMOVED 2026-05-03. What's still missing: pre-write byte-count
             validation gate at the 4 writeFile sites. The 2026-04-30 incident
             (433KB→67KB destructive cut) was on the OLD heuristic; current
             code can still over-cut without a sanity rail. Owned by WP-49.
❌ PARTIAL   Lumberjack stage pipeline — markdown-loaded; typed graph runner owned by WP-51
   PENDING   WP-49  Sage 240/40 pre-write byte-count validation gate
                    (~30 LOC across 4 writeFile sites in lib/memory/dreamer.ts;
                    abort write + preserve snapshot if cut fraction > threshold;
                    deterministic boundary selection ALREADY SHIPPED 2026-04-25,
                    so this is the remaining safety rail not a heuristic rewrite)
...
✅ SHIPPED   WP-13  ToolCard custom-card surface spec — `docs/toolcards-mockup.html`
                    (3,588 lines; 4 archetypes: Job / Diff / Verdict / Control + ambient).
                    No separate TOOLCARD-SPEC.md needed — the mockup IS the spec.
...
❌ PARTIAL   WP-22  ToolCard + 13 custom cards — substrate (CSS) SHIPPED in app/globals.css
                    (64 .tool-card-* rules). Cards SHIPPED (10): BuildJobCard,
                    ApplyPatchCard, ScreenshotCard, UploadEvalCard, UploadEvalBatchCard,
                    ConfirmUnderstandingCard, DiscoveryQuestionsCard (WP-22.DQ — typed
                    fields + universal textarea, 2026-05-08), DesignDirectionsCard
                    (WP-22.DD — 2×3 mood grid, 2026-05-08), GenerateImageCard
                    (Nano Banana → card on image_ready, 2026-05-08), HotSwapCard
                    (before/after diff on hotswap_complete, 2026-05-08).
                    Cards PENDING (~3): Moodboard/DS card (WP-77), descent_selection
                    TWO variants (WP-75), submit_critique radar (WP-54). No JSX
                    <ToolCard> wrapper — substrate is CSS-class only by design.
...
✅ DONE      WP-25  TodoWrite parser + UnfinishedTodosPanel (parser=WP-66's parseTodosSection; tests landed 2026-05-08)
...
❌ PARTIAL   OskarOS deployment story (Linux x86 hosting)
   PENDING   WP-40  CLI-mode Linux portability (~1 session, 11 hardcoded-path call-sites:
             webdev.ts ×4, bridge-process-manager.ts ×2, thumbnail-generator.ts ×1, sentinel-ti.ts ×1,
             webdev/route.ts ×1, probe-model/route.ts ×2). Was 13; -2 from
             /api/claude-code/route.ts deletion (Ralph 2026-05-08 — dead route).
...
✅ SHIPPED   2026-05-08 batch — chat queue + batched mid-stream delivery (queued- id prefix
             stops API-mode message duplication regression in handleSendMessage); SVG <image>
             editing in Director Mode (HTML <img> + SVG <image> normalized via isImageElement /
             readImageSrc / writeImageSrc); 'user' role added to AgentRole + permission table;
             notifyAgent publishes agent_inbox_message event; recursive HTML walk in
             /api/admin/sessions/[id]; /api/claude-code dead route DELETED; Feather.tsx build
             errors fixed; WP-13 closed (toolcards-mockup.html IS the spec, no
             TOOLCARD-SPEC.md ever needed); WP-22 internal cards #1-2 SHIPPED
             (GenerateImageCard + HotSwapCard); WP-22.DD SHIPPED (DesignDirectionsCard);
             WP-22.DQ structured-fields upgrade SHIPPED (typed widgets + universal textarea).
...
(WP-12 ✅ shipped, WP-13 ✅ shipped — `docs/toolcards-mockup.html` IS the spec, WP-14 deferred)
...
**WP-13 — ToolCard custom-card surface spec** — ✅ SHIPPED

The spec is `docs/toolcards-mockup.html` (3,588 lines). It's a runnable HTML
mockup carrying the actual ONYX + POLAR theme tokens, every card archetype's
markup, and every state variant. **The mockup IS the spec** — no separate
TOOLCARD-SPEC.md needed (and never will be; superseded 2026-05-08).

Four archetypes documented in the mockup:

| Archetype | Covers tools |
|-----------|--------------|
| 1 — Job cards | `build_vibe` / `build_all_vibes` / `build_final` / `generate_image` (queued / working / done states with thumbnail) |
| 2 — Diff cards | `apply_patch` / `director_save` / `vibe_diff` (color-highlighted before/after) |
| 3 — Verdict cards | `submit_critique` (radar) / `submit_image_verdict` / `submit_proofread` / `submit_upload_eval` (verdict pill + notes) |
| 4 — Control + ambient | `ask_user` (blocking modal) / `cd_snackbar` (auto-dismiss toast) |

WP-22 (the implementation) consumes this spec directly — see §2.3 for status.
...
**WP-15 — srcdoc font-preload pre-flight audit** ✅ (DONE 2026-05-02)
...
**WP-18 — Hand-rolled markdown renderer** ✅ ⚠️ NEEDS EXTENSIVE TESTING + REDESIGN
...
**WP-20 — AssistantMessage scaffold** PENDING (sequenced after WP-22 pre-pass)
...
**WP-22 — ToolCard + 13 custom cards** ❌ PARTIAL

Substrate is **CSS-class only** (no JSX `<ToolCard>` wrapper) — an intentional
design call: cards inherit `app/globals.css` `.tool-card` / `.tool-card-head`
/ `.tool-card-body` / `.tool-card-foot` / `.tool-card-icon` (with
`data-accent="green|cyan|violet|yellow|red"` variants) / `.tool-card-title` /
`.tool-card-meta` / `.tool-card-summary` / `.tool-card-btn` /
`.tool-card-img-frame` / `.tool-card-ratio-badge` / `.tool-card-readout` and
~50 more rules (64 total). Each card renders its own JSX layout. No parent
component to lift; the substrate is the stylesheet.

**Cards SHIPPED (10 of ~13):**

| Card | File | Archetype | Shipped |
|------|------|-----------|---------|
| `BuildJobCard` | `components/chat/BuildJobCard.tsx` | 1 — Job | 2026-05-06 |
| `ApplyPatchCard` | `components/chat/ApplyPatchCard.tsx` | 2 — Diff | 2026-05-06 |
| `ScreenshotCard` | `components/chat/ScreenshotCard.tsx` | 1 — Job (image-result) | 2026-05-06 |
| `UploadEvalCard` | `components/chat/UploadEvalCard.tsx` | 3 — Verdict | 2026-05-06 |
| `UploadEvalBatchCard` | `components/chat/UploadEvalBatchCard.tsx` | 3 — Verdict (batched) | 2026-05-06 |
| `ConfirmUnderstandingCard` | `components/chat/ConfirmUnderstandingCard.tsx` | 4 — Control | 2026-05-06 |
| `DiscoveryQuestionsCard` | `components/chat/DiscoveryQuestionsCard.tsx` | 4 — Control (WP-22.DQ — typed-fields + universal textarea) | 2026-05-08 (upgrade) |
| `DesignDirectionsCard` | `components/chat/DesignDirectionsCard.tsx` | 4 — Control (WP-22.DD — 2×3 mood grid, cap-2 multi-select) | 2026-05-08 |
| `GenerateImageCard` | `components/chat/GenerateImageCard.tsx` | 1 — Job (Nano Banana → card on `image_ready`) | 2026-05-08 |
| `HotSwapCard` | `components/chat/HotSwapCard.tsx` | 2 — Diff (before/after thumbs on `hotswap_complete`) | 2026-05-08 |

**Cards still PENDING (~3):**

| Card | Tracked under |
|------|---------------|
| Design System / Moodboard | WP-77 (was WP-22.DS) |
| `descent_selection` (TWO variants) | WP-75 (was WP-22.IS) |
| `submit_critique` radar | WP-54 |

**Anti-patterns the substrate already enforces (per §2.3 design pass):**
- Theme awareness via CSS vars — no hardcoded colors per card
- Lucide-style SVG icons (no Unicode glyphs in the implemented cards)
- Inter for prose, JetBrains Mono only for code/diff/path
- Two-row minimum for Tier-2 cards (head + body)

**STATUS: PARTIAL** — substrate done; 10 of ~13 cards shipped; the missing
3 are tracked individually under WP-54 / WP-75 / WP-77. WP-22 closes when
those land.
...
**WP-25 — TodoWrite parser + UnfinishedTodosPanel** ✅ DONE (2026-05-08)
- UI: `components/chat/UnfinishedTodosPanel.tsx` (full bento card, hover-recommend on active rows, hover-delete on completed, minimize/expand) + `components/chat/LiveOverlay.tsx` (mount + SSE subscribe).
- Read-side parser: `parseTodosSection` in `lib/session.ts:563` — turns SESSION.md `## Todos` block into `TodoItem[]`. Glyph mapping `[ ]/[/]/[x]`, optional `(id:STR)` token, cosmetic `~~strikethrough~~` strip. Tolerant: malformed lines / missing section / empty body all yield `[]`.
- Write-side store: `lib/runtime/todos-store.ts` — `writeTodos` (sanitize + auto-fill IDs + splice section + broadcast `todos_updated`) / `readTodos` (auto-backfill missing IDs once per session) / `deleteCompletedTodo` (scoped exception to single-writer rule — refuses non-completed) / `seedDiscoveryTodosIfMissing` (idempotent first-read seed).
- Persistence absorbed into WP-66; the original "parser TBD" entry was stale (ship status never updated when WP-66 landed). The legacy stream-parser at `lib/runtime/todos.ts` is OBSOLETE and kept only for offline replay of pre-2026-05-06 sessions.
- Test coverage (Ralph 2026-05-08 close-out):
  - `lib/session.todos.test.ts` — 22 tests: glyph mapping, id token extract/skip, strikethrough strip (outer-wrap only), tolerance for malformed lines + missing/empty section, ordering, section-boundary detection.
  - `lib/runtime/todos-store.test.ts` — 18 tests: write-round-trip with bus broadcast, ID auto-fill + preservation, status coercion, splice-after-Workflow-State + replace-in-place, read round-trip, one-time backfill behaviour, delete typed errors (`not_found` / `not_completed`), seed idempotency (absent / already-seeded / user-pruned-empty / SESSION.md missing).
  - `lib/runtime/todos.test.ts` — 11 legacy stream-parser tests still green (kept for replay use case).
...
#### Card taxonomy correction (CD, 2026-05-07; revised same day)

Earlier WP-22 specs used "moodboard" as a card name interchangeably for two different surfaces, and treated `descent_selection` as a generic vibe-picker. Both names drifted. The corrected taxonomy locks each card to ONE job (and `descent_selection` does double-duty across two clearly-separated phases). The next three subsections (WP-22.DS / WP-22.DD / WP-22.IS) ship the corrected cards.

| Card name | When fires | What it does | MCP tool | Buttons |
|---|---|---|---|---|
| `discovery` | Phase 1 batched Q&A — ONE card per sub-task per turn (sub-task 0 = user-context, 1-6 = brand canon, 7 = confirm_understanding via separate card). Cold-start = sequence of cards across turns, not a single panel. | Multi-question form supporting the FULL field-type roster (text / textarea / radio / checkbox / select / direction-cards), 3–7 questions per card, brand-anchored framing line ("Tell me about your business. The artifact comes after." — NEVER artifact-anchored), universal "Thoughts, comments, anything else?" textarea at bottom. NEVER asks an artifact-type ("webpage / keynote / brand-cards") question — CD infers from Discovery context. | `ask_discovery_questions` | Submit |
| `design_directions` | Closes Discovery (Phase 1→2). 6 mood seeds, multi-select ≤2. | 6 direction-cards in 2×3 mood.jpg-density grid (label · italic mood · horizontal palette · mono font tags · references) | `present_design_directions` | Commit Directions |
| `descent_selection` (FINAL BUILD) | Phase 4 → 5 first card. N vibes side-by-side, single-select. | N vibe thumbnails: label · sample headline in vibe's display font · 3-swatch palette · school anchor mono caption | `present_descent_selection` | Ship This Vibe |
| `design_system` | Phase 4 → 5 second card. Compact DS at fidelity for the locked vibe; dropdown swaps DS across other Phase-4 vibes. | Compact: header row (icon + title + dropdown), sample panel (display + H2 + body + buttons in vibe's typefaces on vibe's background), palette+typo grid, image+animation grid | `present_design_system` | Select Design System / Create New (cancel-styled secondary, opens DS-creation discovery) |
| `descent_selection` (image strategy) | Phase 3; also Phase 4 → 5 third card. | Image plan visualization. Webpage: vertical stack hero→footer. Keynote: multi-row horizontal (e.g. 20 slides = 4×5). Slots show assigned image OR "GENERATE IMAGE" placeholder (click slot to swap from Assets — no right-side buttons). | `present_image_strategy` | Approve Images / Generate All N (2nd CTA, conditional) |
| `confirm_understanding` | After Discovery; before Build | Summary + Build button | `confirm_understanding` | Build it / Edit |

**The no-Cancel rule (locked 2026-05-07).** Cards do NOT carry Cancel buttons. A card lives in the chat scroll — if the user wants to ignore it, they scroll past or the next message buries it. Cancel is a modal affordance, not a card affordance. Modal `ask_user` and snackbar cancel-job actions are exceptions because they're NOT cards. Action rows on cards: ONE primary CTA, optionally ONE secondary action (cancel-styled, but its label is never literally "Cancel" — it's "Create New" / "Generate All" / etc.).

`moodboard` is RETIRED as a card name. Where it appears in older Feature-X / mockup / agent-doc paragraphs, treat as deprecated and route the use case to one of the cards above.

#### WP-22.DQ ✅ SHIPPED 2026-05-08 — `discovery` card spec (sequence-of-cards Phase 1 Q&A) (CD, 2026-05-07; impl 2026-05-08)

**STATUS:** ✅ SHIPPED. Component upgraded from text-only to full typed-field
roster (text / textarea / radio / checkbox / select). Universal "Thoughts,
comments, anything else?" textarea now present on every card. MCP tool
schema accepts both legacy plain-string questions and structured
DiscoveryQuestion objects. Test suite (7/7) passes.

Files: `lib/types.ts` (DiscoveryQuestion + payload widening),
`components/chat/DiscoveryQuestionsCard.tsx` (rewrite), `mcp-server/
tools-cd.ts` (inputSchema oneOf string/structured), `app/api/mcp/
ask-discovery-questions/route.ts` (defensive coercion), `app/page.tsx`
(passthrough). Spec details below preserved as historical reference.


**Purpose.** The discovery card surfaces ONE sub-task's question batch per turn. Cold-start discovery is a SEQUENCE — 8 sub-tasks (user-context #0 + 7 brand canon) seeded into SESSION.md `## Todos` from `lib/runtime/discovery-seed.ts`. CD fires the card for the current `in_progress` sub-task, reads the answer batch, optionally fires ONE clarifying-round card if answers leave gaps, then advances to the next sub-task. **No discovery card asks an artifact-type ("webpage / keynote / brand-cards") question** — CD infers the build kind from Discovery context and fires the matching build tool at the `confirm_understanding` gate.

**Earlier draft (now superseded).** A previous render shipped with five questions in one panel, the framing line `"Cold start. Tell me what we're making."`, and an artifact-type radio at Q2. All three were doctrine violations: (a) cold-start needs MANY questions across MANY turns, not 5 in one panel; (b) the framing line anchored in artifact, propagating the wrong frame to the rest of the card; (c) Q2 artifact-type radio directly contradicts workflow.md Path A ("Don't ask 10 questions about what artifact to build before you've heard the brand"). Retired 2026-05-07. Subsequent draft kept the artifact-type as a "track lock" radio on `confirm_understanding`; that too has been retired (2026-05-08) — the artifact kind is purely inferred, never user-radioed.

**Source tool.** `ask_discovery_questions(questions[], context?)` — already in MCP. Returns user answers as a regular user message in the next turn.

**Card payload (props).**

```ts
type DiscoveryCardProps = {
  subTaskId: 'user-context' | 'basics' | 'weird-detail' | 'signature-experience'
            | 'enemy' | 'real-customer' | 'offerings'
  // sub-task 'confirm-understanding' uses confirm_understanding card, not this one
  context: string                      // brand-anchored framing line, e.g. "Tell me about your business. The artifact comes after."
  questions: Array<{
    id: string                         // stable identifier
    label: string                      // question text
    type: 'text' | 'textarea' | 'radio' | 'checkbox' | 'select' | 'direction-cards'
    options?: string[]                 // for radio/checkbox/select/direction-cards
    required?: boolean
    placeholder?: string
    help?: string
  }>
  totalQuestions: number               // for the "{n} questions" counter — typically 3-7 per card
}
```

**Rendered structure.**

1. **Card header** — `?` icon · "A few quick questions" title · `{totalQuestions} questions` mono uppercase counter (right).
2. **Context line** — accent-bar (left, 2px primary) · brand-anchored framing line in `context`. Examples per sub-task:
   - sub-task 0 (user-context): `"Before I sketch anything — tell me about you. What's your role and what do you want from this work?"`
   - sub-task 1 (basics): `"Tell me about your business. The artifact comes after."`
   - sub-task 2 (weird detail): `"Now the part you almost don't say. What surprises people about this place?"`
   - sub-task 3 (signature experience): `"What's the moment customers remember after they leave?"`
   - sub-task 4 (enemy): `"What does your industry get wrong? What would you never do?"`
   - sub-task 5 (real customer): `"Pick one customer. One real person. Describe them."`
   - sub-task 6 (offerings): `"What can people book, what does it cost, and what's the signature thing?"`
3. **Questions** — 3–7 form fields, each rendered per its `type`. MCQ types (radio / checkbox / select / direction-cards) are first-class — never use plain `text` for what should be MCQ.
4. **Universal textarea** — top-border separator, mono-uppercase prompt `THOUGHTS, COMMENTS, ANYTHING ELSE?`, textarea min-height 56px. NEVER omitted.
5. **Action row** — SINGLE button:
   - `Submit answers` (primary CTA) — fires `ask_discovery_questions → submit_answers(answers, freeformContext)`.

NO Cancel — it's a card.

**Cadence.**

- ONE card per turn. CD reads answers, optionally fires ONE clarifying-round card (also a `discovery` card with the gaps as questions), then advances to the next sub-task.
- On cold start with NO `user.md` portrait, sub-task 0 fires first and routes answers to user.md (or queues for Sage).
- On a session WITH an existing `user.md`, sub-task 0 can be flipped to `completed` immediately on read — no card fires. CD jumps to sub-task 1.

**Implementation files.**
- `components/chat/cards/DiscoveryCard.tsx` — already exists; needs framing-line + field-type-roster + universal-textarea + sequence-of-cards architecture
- `lib/runtime/discovery-seed.ts` — UPDATED 2026-05-07 with user-context as item #0 (8 items total)
- MCP server: `ask_discovery_questions` already wired

**Done means.**
- [x] Card supports the full field-type roster (text / textarea / radio / checkbox / select), not text-only — direction-cards intentionally OUT (lives on `present_design_directions` per WP-22 doctrine collapse)
- [x] Framing line is brand-anchored, never artifact-anchored — `context` prop, no artifact-type wording
- [x] Universal textarea at the bottom of every card — "Thoughts, comments, anything else?", non-omittable
- [x] NO Cancel button rendered — Submit-only foot
- [x] NO artifact-type question at any sub-task (CD infers build kind from Discovery)
- [x] Counter reflects actual question count for the current sub-task
- [x] Sequence cadence works: one card per turn, optional clarifying round, then advance — CD-side discipline, doctrine-anchored in agent prompt + workflow.md
- [x] Sub-task 0 (user-context) fires first on cold start; skipped on sessions with existing user.md — `discovery-seed.ts` item #0 + agent doctrine
- [x] Theme-aware — uses CSS variables throughout

**Status: SHIPPED 2026-05-08.** Verified via `npx vitest run components/chat/DiscoveryQuestionsCard.test.tsx` (7/7 green) + `tsc --noEmit` (clean). Files touched: `mcp-server/tools-cd.ts` (typed schema), `app/api/mcp/ask-discovery-questions/route.ts` (typed validation), `app/page.tsx` (typed event handler), `components/chat/DiscoveryQuestionsCard.tsx` (full field roster + universal textarea + required-validation), `lib/types.ts` (DiscoveryQuestion interface), `components/chat/DiscoveryQuestionsCard.test.tsx` (back-compat + structured-shape tests).

#### WP-22.DS — Design System card spec (MOODBOARD-SINGLE) (CD, 2026-05-07)

**Purpose.** The Design System card is the Phase 4 → Phase 5 sign-off surface. It renders the FULL design-system spec for ONE vibe (vs the multi-option `design_directions` card). User reviews the system, optionally tweaks via the universal textarea, then sets it as the default that the Final Build inherits.

**Source tool.** `present_design_system(vibeSlug)` — new MCP tool to land with WP-22. User actions returned via `select_design_system(slug)` (lock the DS for `build_final`) OR `create_design_system()` (forks into DS-creation discovery sub-flow). NO `tweak` and NO `cancel` actions — the textarea contents post back automatically as user message regardless of which button fires. Companion live mockup: `docs/toolcards-mockup.html` § "DESIGN SYSTEM" (compact variant, locked 2026-05-07).

**Card payload (props).**

```ts
type DesignSystemCardProps = {
  vibes: Array<{                       // ALL Phase-4 vibes for the dropdown
    slug: string                       // "vibe-3-grandmas-cliff"
    label: string                      // "Vibe-3 — Grandma's Cliff"
    displayName: string                // "Grandma's Cliff" — rendered in vibe's display font
    forWho: string                     // "Saudi 35-55, highland lineage..."
    mood: string                       // "Warm, oracular, nostalgic, restrained, guilt-inducing"
    atmosphere: string                 // one-sentence quote rendered italic w/ accent border
    palette: Array<{                   // 5 swatches — Background, Surface, Primary, Ink, Accent
      hex: string
      role: 'Background' | 'Surface' | 'Primary' | 'Ink' | 'Accent'
    }>
    typography: {
      displayFont: string              // "Inter Tight"
      bodyFont: string                 // "Inter"
      h1Sample: string                 // "Grandma's Waiting"
      h2Sample: string                 // "She's already made too much food"
      bodySample: string               // body paragraph
    }
    components: {
      primaryButtonLabel: string       // "Reserve a seat"
      secondaryButtonLabel: string     // "Read more"
    }
    imageTreatment: string             // "Hero: full-bleed, object-fit: contain..."
    animation: string                  // "Static. No scroll-coupled motion..."
  }>
  selectedSlug: string                 // which vibe's DS is shown initially
  trackLabel: string                   // "webpage" | "keynote" | "brand-cards"
}
```

**Rendered structure (top to bottom). Compact — fits within ~3-4 viewport heights, not a full page.**

1. **Header row (single line)** — DS icon · "Design System" title + "Phase 4 → 5 sign-off · Track: {trackLabel}" mono caption · `<select>` vibe-selector inline (right-aligned). The dropdown is in the header — no separate "SHOW DS FOR" row.
2. **Sample panel** — rendered on the vibe's `Background` color with `Surface`-color border, padded 14×16. Renders three live samples in the vibe's actual typefaces, plus inline component samples:
   - H1 sample (28px `displayFont` 800 UPPERCASE in `Primary`)
   - H2 sample (18px `displayFont` 800 sentence-case in `Ink`)
   - Body sample (12px `bodyFont` 400 / 1.55 in `Ink`)
   - Inline button row: primary CTA (`Primary` bg, body font 700) + ghost secondary (`Primary` border + text) — both labeled per `vibes[].components`.
3. **Palette + Typography grid** — 2-column horizontal row:
   - Left cell: `PALETTE` mono caption · 5-cell horizontal swatch strip (24px tall) · 5-cell hex caption row (mono 8px). Order: Background, Surface, Primary, Ink, Accent.
   - Right cell: `TYPOGRAPHY` mono caption · `{displayFont} · {bodyFont}` · captions `H1 · 48px 800 UPPER` and `Body · 16px 400 · 1.6`.
4. **Image treatment + Animation** — 2-column grid, each cell: mono uppercase header + 11px copy. Compact (8×10 padding).
5. **Universal user-input textarea** — top-border separator, mono-uppercase prompt, textarea min-height 56px.
6. **Action row** — TWO buttons:
   - `Select Design System` (primary CTA, brand-green-bright bg) — fires `present_design_system → select_design_system(selectedSlug)`. The selected vibe's DS becomes the canonical baseline; `build_final` inherits it.
   - `Create New` (cancel-styled secondary, ghost border) — fires `present_design_system → create_design_system()`. CD drops into the DS-creation discovery sub-flow.

NO Tweak, NO Cancel. Textarea content always posts back regardless of which button fires.

**Why no Tweak.** "Tweak" was an extra round-trip that didn't earn its weight — the textarea already round-trips freeform feedback. Either the user picks the DS (Select), or they reset (Create New); freeform notes ride along either path.

**Interactivity.** The vibe-selector dropdown changes ALL sections below it client-side from the pre-loaded `vibes[]` array. No re-fetch. The `selectedSlug` state is local to the card. On `Select Design System`, the chosen slug + textarea contents fire to the MCP server. On `Create New`, no slug — CD enters DS-creation discovery.

**Companion to other discovery cards.**
- `descent_selection` (FINAL BUILD variant) fires BEFORE this card — picks WHICH vibe ships. This card surfaces second to lock the DS.
- `descent_selection` (image-strategy variant, WP-22.IS) fires AFTER this card — third in the Phase 4→5 sequence, showing the locked vibe's image plan.

**Implementation files.**
- `components/chat/cards/DesignSystemCard.tsx` — NEW (~220 LOC, compact)
- `lib/runtime/cards.ts` — register `design_system` tool name → card component
- MCP server: `present_design_system` tool + `select_design_system` / `create_design_system` callbacks

**Done means.**
- [ ] Compact layout fits within ~3-4 viewport heights (vs the original full-page draft)
- [ ] Vibe-selector dropdown is in the card header, not a separate row
- [ ] Sample panel renders H1/H2/body samples in the vibe's display+body fonts on the vibe's bg/ink colors, with inline primary+ghost button samples
- [ ] Palette + Typography are side-by-side compact cells (not a full-width 5-column grid)
- [ ] Dropdown change updates every section client-side without server hit
- [ ] Select Design System fires the MCP tool with `selectedSlug` + textarea content
- [ ] Create New fires the DS-creation discovery sub-flow
- [ ] NO Tweak button rendered
- [ ] NO Cancel button rendered
- [ ] Theme-aware (works in OD-light + OD-dark per WP-22 token system)
- [ ] Visual regression baseline captured (WP-24)

#### WP-22.DD ✅ SHIPPED 2026-05-08 — `design_directions` card spec (close-of-Discovery direction picker) (CD, 2026-05-07; impl 2026-05-08)

**STATUS:** ✅ SHIPPED. New `present_design_directions(directions[],
trackLabel?)` MCP tool wired through allowlist + dispatcher + new route at
`/api/mcp/present-design-directions`. Card renders 2×3 mood grid at
mood.jpg density (label · italic mood · 4-swatch palette strip · mono
display·body font tag · references list). Cap-2 multi-select with
red-flash warning when blocked. Universal textarea always present. Single
"Commit Directions" CTA, no Cancel. User selection commits as markdown
user message ("Picked directions: dir-X, dir-Y. Notes: …"). 8 files
touched; type-check clean. Spec details below preserved as historical
reference.


**Purpose.** The Design Directions card closes Discovery (Phase 1 → Phase 2). User picks up to 2 mood directions that resonate from a curated set distilled by CD from Discovery answers. These aren't final — they're seeds for the N vibes CD will write in Phase 4. The Phase 4 → 5 vibe-pick gate is served by `descent_selection` (FINAL BUILD variant, WP-22.IS) instead — NOT this card.

**Source tool.** `present_design_directions(directions[])` — new MCP tool. `maxSelect` is fixed at 2 (close-of-discovery). Returns user choice via `commit_directions(slugs[], freeformContext?)`.
...
type DesignDirectionsProps = {
  directions: Array<{
    slug: string                       // "dir-luxury", "dir-warm", ...
    label: string                      // "Luxury — Hermès / Aman"
    mood: string                       // "Hushed, exclusive, every detail considered."
    references: string[]               // up to 4: "Hermès", "Aman", "The Carlyle"
    palette: string[]                  // 4 hex codes (mood.jpg density: 4-swatch horizontal strip)
    displayFont: string                // mono caption display name, e.g. "Didot"
    bodyFont: string                   // "Inter"
  }>
  trackLabel: string                   // "webpage" | "keynote" | "brand-cards"
}
...
**Rendered structure (mood.jpg density, 2026-05-07).**

1. **Card header** — DD icon · "Design Directions" title · sub-label `Closes Discovery · Track: {trackLabel}` · `PICK ≤2` pill.
2. **Intro line** — short prose: "These aren't final — they're seeds for the 5 vibes I'll write. Pick up to two that resonate."
3. **2×3 direction-card grid** — 6 directions in a 2-column × 3-row layout. Each direction card:
   - Label (body font 700, 14px) — e.g. "Luxury — Hermès / Aman" — full first line.
   - Italic mood line (11.5px muted, 1.45 line-height) — e.g. "Hushed, exclusive, every detail considered."
   - **Horizontal 4-swatch palette strip** (14px tall, 3px border-radius, full-card-width).
   - Mono `display · body` font tag (9.5px uppercase muted, `·` separator) — e.g. "DIDOT · INTER".
   - References list (11.5px body font muted, `·` separator) — e.g. "Hermès · Aman · The Carlyle".
   - Selected card gets a 2px primary border + green ✓ pill upper-right.
4. **Selection counter** — `Selected: {n} / 2` mono uppercase, with cap warning ("Deselect one to swap") that flashes red when blocked.
5. **Universal textarea** — placeholder example: "Warm + Eastern, but pull the Editorial typography discipline into both."
6. **Action row** — SINGLE button:
   - `Commit Directions` (primary CTA, brand-green-bright bg) — disabled until ≥1 card selected. Fires `commit_directions(slugs, freeformContext)`.

NO Cancel — it's a card. If the user wants to ignore it, they scroll past or send a normal chat message instead.

**Interactivity.** Multi-select up to 2. Cap-enforced client-side: clicking an unselected card when at cap flashes the warning red AND blocks selection until user deselects another. Source: companion mockup at `docs/toolcards-mockup.html` § "DESIGN DIRECTIONS" lines locked 2026-05-07.

**Reference image.** `public/2026-01-27-debug/mood.jpg` (uploaded 2026-05-07) — 4-card UI screenshot demonstrating the target density: label · italic mood · 4-swatch horizontal palette strip · `display · body` font tag · references list. Production card matches this density at 2×3 grid (6 options instead of 4).

**Implementation files.**
- `components/chat/cards/DesignDirectionsCard.tsx` — NEW (~200 LOC, compact), reuses `DirectionCard` primitive from `question-form` direction-cards type
- `lib/runtime/cards.ts` — register `design_directions` tool name
- MCP server: `present_design_directions` tool + `commit_directions` callback

**Done means.**
- [ ] 2×3 grid renders 6 directions at mood.jpg density
- [ ] Each card shows: label, italic mood, horizontal 4-swatch palette strip, mono `display · body` font tag, references list
- [ ] Multi-select cap-2 enforced client-side; cap warning flashes red on blocked click
- [ ] COMMIT disabled until ≥1 selection
- [ ] NO Cancel button rendered
- [ ] Universal textarea always present
- [ ] Theme-aware

#### WP-22.IS — `descent_selection` card spec — TWO variants (CD, 2026-05-07)

**Purpose.** `descent_selection` does double-duty across two clearly-separated phases. Both variants share the card chassis but differ in payload + behavior:

- **Image-strategy variant** (Phase 3 + Phase 4→5 third card): visualizes the image plan for ONE vibe. Webpage = vertical stack (hero → footer). Keynote = horizontal multi-row (e.g. 20 slides = 4×5). Each slot shows assigned image OR "GENERATE IMAGE" placeholder (click slot to swap from Assets panel — NO right-side buttons; the slot itself is the click target).
- **FINAL BUILD variant** (Phase 4→5 first card): N vibes side-by-side, single-select. User picks the ONE vibe that ships. Each thumbnail = label · sample headline in vibe's display font · 3-swatch palette · school anchor mono caption.

**Source tools.** Two MCP tools, both surface `descent_selection` cards:
- `present_image_strategy(vibeSlug, layout, slots[])` → image-strategy variant. Returns user actions via `swap_image_slot(slotId, assetFilename)`, `generate_all_missing(slotIds[])`, `approve_images(vibeSlug)`.
- `present_descent_selection(vibes[], maxSelect=1)` → FINAL BUILD variant. Returns user pick via `commit_final_vibe(vibeSlug, freeformContext?)`.

**Earlier name.** Previously called "Descent Selection" with a different scope (25-card brand-cards starring matrix multi-pick). That role still exists for brand-cards but is now one of three patterns this card supports — the other two being image-strategy (Phase 3) and FINAL BUILD vibe-pick (Phase 4→5).

##### IMAGE-STRATEGY VARIANT — Card payload (props)

```ts
type ImageStrategyProps = {
  vibeSlug: string                     // "vibe-3-grandmas-cliff"
  vibeLabel: string                    // "Vibe-3 — Grandma's Cliff"
  layout: 'webpage-vertical' | 'keynote-multi-row'
  slots: Array<{
    id: string                         // "hero", "menu-bg", "slide-7", "slide-12"
    sectionLabel: string               // "Hero" / "Menu" / "Slide 7 — Q3 Pipeline"
    purpose: string                    // "hero", "section-bg", "portrait", "icon", "data-visual"
    aspectRatio: string                // "16:9", "3:4", "1:1"
    state: 'assigned' | 'placeholder' | 'optional-empty'
    assignedFilename?: string          // when state=assigned
    placeholderPrompt?: string         // when state=placeholder, the Nano prompt CD has drafted
  }>
  columnsPerRow?: number               // default 5 for keynotes
}
```

##### IMAGE-STRATEGY VARIANT — Rendered structure

1. **Card header** — IS icon · "Image Strategy" title · sub-label `{vibeLabel} · Layout: {layout} · {phase}` · `REVIEW` pill.
2. **Assets panel pointer** — short prose: "Click any GENERATE IMAGE slot to fire it from the Assets panel — or generate them all at once. Optional sections are dimmed — they don't need an image."
3. **Slot grid** — branches on `layout`:
   - **`webpage-vertical`:** stacked vertical layout, top-to-bottom mirroring the page sections (hero → hook → how-it-works → menu → location → footer). Each slot row: 96×54 thumbnail (left) · `sectionLabel` + `{aspectRatio} · {purpose}` mono caption + (when placeholder) prompt preview italic. NO right-side buttons — the entire row is the click target. Placeholder thumbnail reads "GENERATE IMAGE" in mono uppercase yellow on dashed-yellow border. `optional-empty` state: dimmed at 0.55 opacity with "—" placeholder thumb and "no image needed" caption.
   - **`keynote-multi-row`:** flex-wrap horizontal grid, `columnsPerRow` slots per row (default 5). 20 slides → 4 rows × 5. Each cell is 16:9 thumbnail with `slide-{N}` mono caption above. Same assigned/placeholder/optional-empty states. Placeholder cell reads "GENERATE IMAGE" in mono uppercase yellow.
4. **Stats bar** — `Assigned: {n} · Generate: {m} · Optional/Type-only: {k}` mono uppercase.
5. **Universal textarea** — placeholder examples per use case.
6. **Action row — button hierarchy is CONDITIONAL on placeholder count:**
   - **If ≥1 placeholder exists (the common case during Phase 3 + Phase 4→5 review):**
     - `Generate All Images N` (primary green CTA, full-width flex:1) — fires `generate_all_missing(slotIds)`. Each placeholder transitions to "generating" state; card subscribes to `image_ready` events and updates matching slots in place. Generation is the load-bearing action when work remains; that's why it's primary.
     - `Approve Images` (secondary, ghost / cancel-styled with `border-card` border + `text-muted` text) — fires `approve_images(vibeSlug, textareaContents)`. The user can ship even with placeholders if they intend to fill them later or accept the empty slots — but it's the alt-path when generation is available.
   - **If all images assigned (no placeholders, terminal state after generation completes OR user has manually swapped every slot):**
     - `Approve Images` (single primary green CTA, full-width) — fires `approve_images(vibeSlug, textareaContents)`. Generate All disappears entirely.

NO Cancel — it's a card.

Why the swap. The user lands on this card when CD has just laid out the image plan. The first action they want to take is "fire the missing generations" — that's what moves the work forward. Approve-with-placeholders is the alt-path (acknowledged-trust shortcut). Once generation is done and every slot is filled, Approve becomes the only meaningful action and gets promoted back to primary.

##### IMAGE-STRATEGY VARIANT — Interactivity

- Click anywhere on a placeholder slot row/cell → opens the Assets panel pre-filtered to images matching the slot's `purpose` + `aspectRatio`. User picks an asset; client fires `swap_image_slot(slotId, filename)`. Card updates in place.
- Click anywhere on a placeholder slot when no asset matches → fires `generate_image(prompt, ratio, slot=slotId)` for that single slot.
- Real-time updates via `image_ready` events on the replay_events bus.

##### FINAL BUILD VARIANT — Card payload (props)

```ts
type DescentFinalBuildProps = {
  vibes: Array<{
    slug: string                       // "vibe-3-grandmas-cliff"
    label: string                      // "Vibe-3 — Grandma's Cliff"
    sampleHeadline: string             // "Grandma's waiting." — rendered in displayFont
    displayFont: string                // "Inter Tight"
    palette: string[]                  // 3 hex codes (bg / primary / accent)
    schoolAnchor: string               // "Soho House" / "Stamen" / "Field.io" / etc.
  }>
  trackLabel: string                   // "webpage" | "keynote" | "brand-cards"
}
```

##### FINAL BUILD VARIANT — Rendered structure

1. **Card header** — DS icon · "Descent Selection · FINAL BUILD" title · sub-label `Phase 4 → Phase 5 · Track: {trackLabel} · {n} vibes` · `PICK ONE` pill.
2. **Intro line** — short prose: "Five vibes built and reviewed. Pick the one that ships — the others archive as alternates."
3. **Horizontal vibe-thumb grid** — N columns (5 for webpages, 2 for keynotes Editorial+Interactive). Each thumb:
   - Top portion (100px tall): rendered with vibe's `palette[bg]` background, `Vibe-{N}` mono caption upper-left, `sampleHeadline` rendered in `displayFont` `palette[primary]` color.
   - Bottom portion: label (body font 600 11px), 3-swatch mini-palette strip, `schoolAnchor` mono caption uppercase.
   - Selected thumb gets 2px primary border + green ✓ pill upper-right.
4. **Universal textarea** — placeholder example: "ship Vibe-3 — but pull the Vibe-1 hook copy into it."
5. **Action row** — SINGLE button:
   - `Ship This Vibe` (primary CTA) — disabled until a vibe is picked. Fires `commit_final_vibe(vibeSlug, freeformContext)`.

NO Cancel.

##### Companion sequence (Phase 4 → 5)

The three Phase 4→5 cards fire in this order:
1. `descent_selection` (FINAL BUILD variant) — pick ONE vibe.
2. `design_system` (WP-22.DS) — lock the DS for that vibe.
3. `descent_selection` (image-strategy variant) — review and approve images for the locked vibe.

Then `build_final` MCP tool fires.

##### Implementation files

- `components/chat/cards/DescentSelectionCard.tsx` — NEW (~280 LOC, dispatches on variant)
- `components/chat/cards/ImageSlotRow.tsx` (vertical layout) + `ImageSlotCell.tsx` (multi-row layout) + `VibeThumb.tsx` (FINAL BUILD layout) — NEW shared primitives (~120 LOC each)
- `lib/runtime/cards.ts` — register `descent_selection` tool name
- MCP server: `present_image_strategy` + `present_descent_selection` + `swap_image_slot` + `generate_all_missing` + `approve_images` + `commit_final_vibe`

##### Done means

- [ ] Image-strategy webpage-vertical layout renders for webpage tracks
- [ ] Image-strategy keynote-multi-row layout renders for keynote tracks (5-col flex-wrap)
- [ ] Slot click (placeholder) opens filtered Assets panel; swap fires the MCP tool and updates card in-place — NO right-side buttons rendered on slot rows
- [ ] Button hierarchy is conditional on placeholder count: if ≥1 placeholder exists, "Generate All Images N" is the primary green CTA and "Approve Images" is the secondary ghost; if all assigned, only "Approve Images" shows as the single primary CTA
- [ ] Generate All Images N fires `generate_image` for every placeholder; card transitions slots to "generating" and listens for `image_ready`; once all complete, button row re-renders with Approve as the single primary CTA
- [ ] FINAL BUILD variant renders N vibe thumbs with display-font sample, palette, school anchor
- [ ] FINAL BUILD single-select fires `commit_final_vibe`
- [ ] Stats bar reflects live counts in image-strategy variant
- [ ] NO Cancel button rendered in either variant
- [ ] Theme-aware
- [ ] Visual regression baselines captured for all three layouts (web-vertical / keynote-multi-row / FINAL BUILD) (WP-24)
...
**WP-28 — Reporter format pre-pass** PENDING
...
**WP-29 — Route-mocking helpers** ❌ PARTIAL
...
**WP-31 — Popover visual baselines** PENDING
...
**WP-33 — Modal visual baselines** PENDING
...
**WP-34 — hyperframes integration** PENDING
...
**WP-35 — SketchEditor integration** ✅
...
## §3 — Junior Designer Workflow Integration
...
### §11.1 WP-49 — Sage 240/40 pre-write byte-count validation gate

**Status correction (2026-05-08):** the "heuristic Block summarization" the
prior version of this block proposed to replace is already gone. Two prior
rewrites delivered the deterministic part:

1. **2026-04-25 architectural rewrite** (Ralph): JS does ALL mechanical work
   in `lib/memory/dreamer.ts:_runSage240_40Inner`. Find anchor (first User
   turn after last Block), take 200 lines forward, snap to next
   `---\n#### User|CD` boundary within +100 lines, splice. Agent's only job
   is "compress this 200-line dialogue chunk into a Block-narrative" via
   `callAnthropic` (text-in / text-out, no tools, ~30-60s). Boundary
   selection is JS-deterministic.
2. **2026-04-30 mitigation:** 24h pre-prune snapshot retention added
   (`SAGE_SNAPSHOT_TTL_MS`). Prior policy of "delete snapshot on cut
   success" had erased the rollback path on the day of the destructive
   incident; new policy keeps snapshots for 24h regardless of cut outcome.
3. **2026-05-03 cleanup:** time-based `findMostRecentBlockEndTime` /
   `findLiveTissueLine` protection REMOVED — was trapping Sage when the
   most-recent dated Block was stale. Now: simple geometric `sectionEnd =
   last-200-lines protection`.

**What's still missing — the actual scope of WP-49:**

`lib/memory/dreamer.ts` has 4 unguarded `writeFile(sessionMdPath, ...)`
sites (lines 494, 936, 1676, 1972). **No pre-write byte-count check at any
of them.** If the splice + agent narrative produces a `finalContent` that's
catastrophically smaller than `inputSize` — the exact 433KB → 67KB pattern
of the 2026-04-30 incident — nothing stops the overwrite. The 24h snapshot
is recovery-after-the-fact, not prevention.

**Spec — ~30 LOC across 4 sites:**

```ts
const inputBytes = Buffer.byteLength(sessionMd, 'utf-8')
const outputBytes = Buffer.byteLength(finalContent, 'utf-8')
const cutFraction = (inputBytes - outputBytes) / Math.max(inputBytes, 1)
const MAX_SINGLE_CUT_FRACTION = 0.30   // tunable; >30% drop is suspect

if (cutFraction > MAX_SINGLE_CUT_FRACTION) {
  console.error(
    `[sage-240-40] ABORTING WRITE: cut fraction ${cutFraction.toFixed(2)} ` +
    `(${inputBytes} → ${outputBytes}). Snapshot at ${snapshotPath} preserved.`,
  )
  emit({ agent: 'lumberjack', phase: 'aborted',
         detail: `cut too large: ${(cutFraction * 100).toFixed(0)}%` })
  trace('cut.write.aborted-oversized', { inputBytes, outputBytes, cutFraction })
  return { kind: 'aborted', reason: 'oversized cut' }
}
```

Apply at 4 writeFile call sites in dreamer.ts. The Compact pass (line 1972)
needs a slightly different threshold because Compact INTENTIONALLY collapses
3 Blocks → 1 (smaller output is the goal); use `MAX_COMPACT_FRACTION = 0.50`.

**Test scenarios (WP-49 acceptance):**
- 433KB input → 67KB output → ABORT (cutFraction 0.85 > 0.30); snapshot kept
- 270KB input → 240KB output → OK (cutFraction 0.11 < 0.30)
- 270KB input → 250KB output → OK (cutFraction 0.07 < 0.30)
- Compact pass: 240KB → 200KB → OK on 0.50 threshold (cutFraction 0.17)
- Compact pass: 240KB → 100KB → ABORT (cutFraction 0.58 > 0.50)
- aborted run: subsequent Sage run reads snapshot, retries cleanly
- emit channel: `phase: 'aborted'` reaches CompactionOverlay UI
...
## §19 — 5-Phase model + new build tools (2026-05-07)

CD's workflow restructure: replace the flat 4-vibe rule with a track-aware 5-phase model. New top-level TodoList: `Discovery → Wireframe → Image Strategy → Generate Vibes → Final Build`. Phase 2 (Wireframe) is exploratory and cheap; Phase 3 (Image Strategy) sequences after Wireframe so the wireframe pass tells you which slots actually need generation; Phase 4 (Generate Vibes) is committed and school-anchored; Phase 5 is the master. User Selects is NOT a phase — it's a gate (the Descent Selection ToolCard) inside Phase 4 → Phase 5.

The full doctrine update landed in `agents/creative-director-agent.md` § "WHAT YOU DELIVER — 5-Phase Model" (replaces the prior 3-line deliverable list) and `agents/webdev-agent.md` § "The Mission" (track + phase strictness table). This §18 lists the infrastructure work needed to support it.
...
| 2 — Wireframe | 3 wireframes (no school) | 5 vibes × 3 slides | 25 cards (20 schools + 5 CD) |
| 3 — Image Strategy | use-as-is / modify / generate per slot | same | same |
| 4 — Generate Vibes | 5 vibes (3-of-5 school-anchored) | 2 vibes (Editorial + Interactive, many slides) | user-starred subset (~7) |
| 5 — Final Build | 1 master page | 1 master keynote | 1 card in Branding |

### WP-78 — `build_wireframes` MCP tool (NEW)
...
Pre-req: §17 hygiene gate — every new tool requires CD allowlist patch + agent respawn. Document in WP-78's done-means checklist.
...
Pre-req: same allowlist + respawn gate as WP-78.
...
The Confirm Understanding card surfaced at the Phase 1 → Phase 2 gate gets a `track` field: radio with three options (`webpage | keynote | brand-cards`). CD pre-fills based on Discovery answers; user can flip before clicking Build. The clicked Build button fires the matching Phase 2 tool (WP-78 / WP-70 / WP-71).
...
Tier 3 specialty panel, **larger than Moodboard**. Where Moodboard surfaces N parallel options, Design System surfaces ONE vibe's full DS in detail and lets the user flip across all sibling vibes' DSes from a single card.

**Card anatomy (locked 2026-05-08):**

1. **Vibe selector dropdown** — at the top of the card body. Lists all Phase 4 vibes (e.g. 5 entries for webpages, 2 for keynotes). User picks a vibe; the card swaps in that vibe's DS payload. The dropdown is the "show-me-vibe-N's-system" switch — the same card surface, different data.
2. **Vibe meta panel** — display name in the vibe's display font (e.g. Inter Tight 800 24px), followed by `For: <persona>` and `Mood: <adjectives>` rendered in the vibe's body font. This pairing demonstrates the actual typeface combo the way the gallery does.
3. **Atmosphere statement** — italic quote-block, brand-green left border. One sentence describing the mood the rest of the system serves.
4. **Color palette** — 5-swatch row with hex codes + role labels (Background / Surface / Primary / Ink / Accent).
5. **Typography spec block** — H1 / H2 / Body samples on the vibe's bg color, in the vibe's actual fonts. Each sample annotated with its size + weight + letter-spacing spec underneath in mono.
6. **Components row** — button primary, button secondary, card chassis sample (in the vibe's tokens).
7. **Image treatment + Animation** — paired panels. Image treatment = crop/contain/overlay rules. Animation = motion posture (static / restrained / scroll-coupled) with easing + duration if applicable.
8. **Universal user-input textarea** — "Thoughts, comments, anything else?" (per WP-73).
9. **Action row** — three buttons: `Set Default` (primary — locks the currently-displayed vibe's DS as the canonical baseline that drives Phase 5 Final) / `Tweak` (secondary — opens the textarea path, signals CD to adjust before locking) / `Cancel` (tertiary — returns to multi-option moodboard or descent card).

The Do's/Don'ts grid was specced in an earlier draft and removed 2026-05-08 — those constraints live in the Design System block in CREATIVE-BRIEF.md / VIBE-N.md and don't need surface real estate on the card.

**Surfaces at three moments:**
1. Phase 2 → Phase 4 alt-mode: when the user wants to commit to a single design system without seeing all Phase 4 vibes built (skips the multi-option moodboard, locks one direction as the baseline for all Phase 4 vibes).
2. Phase 4 → Phase 5 sign-off: after `descent_selection` picks a vibe, the Design System card surfaces that vibe's full DS plus the dropdown to compare against sibling vibes before locking. `Set Default` advances to Final.
3. Mid-iteration: CD wants explicit confirmation on a single DS proposal before deviating.

**Full-width treatment.** This card lifts the 540px chassis cap (per "table-format cards earn full-width treatment" doctrine, locked 2026-05-06) — the DS spec + vibe selector + components row don't fit at narrow widths.

Files: `components/chat/DesignSystemCard.tsx` ~220 LOC. MCP tool `surface_design_system({ slug, vibes[], defaultVibeIndex })` ~50 LOC. Each entry in `vibes[]` mirrors the Design System block + the Vibe Meta block (display-name + For + Mood) CD writes in CREATIVE-BRIEF.md / VIBE-N.md. The dropdown switches between entries client-side without a re-fetch.
...
### WP-76 — Discovery seed: 5-phase wrapper + sub-task structure

`lib/runtime/discovery-seed.ts` extension. Top-tier 5 items (`Discovery / Wireframe / Image Strategy / Generate Vibes / Final Build`) are seeded alongside the existing 7 Discovery sub-tasks. The 5 top-tier items flip pending → in_progress → completed as CD advances. Sub-tasks for Wireframe / Image Strategy / Generate Vibes / Final Build are added by CD when entering the phase, scoped to the build kind CD inferred from Discovery.
...
WP-78 / WP-70 / WP-71 (new build tools) ship next — gated on §17 allowlist hygiene per tool.
